...
const createCSVError = require('./createCSVError');
...
async function convertAllToCSV(file, folderName, numberOfFile) {
  ...
  for (let index = 0; index < parseInt(numberOfFile); index++) {
      ...
      let datas = await readFileError(folderName + fileName).then(datas => datas);
      console.log('Finish parsing. Total chunks: ' + datas.length);
      console.log('Total lines: ' + datas.length)
      await createCSVError(index, datas, file);
